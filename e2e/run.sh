...
cleanup() {
    echo "--- cleanup ---"
    limactl shell "$VM_NAME" sudo pkill -f ipneigh_exporter 2>/dev/null || true
    limactl shell "$VM_NAME" sudo ip netns del e2e-ns 2>/dev/null || true
}
...
GOARCH=$(limactl shell "$VM_NAME" uname -m 2>/dev/null | sed 's/aarch64/arm64/;s/x86_64/amd64/')
...
HEALTH=$(limactl shell "$VM_NAME" curl -s -o /dev/null -w '%{http_code}' "http://localhost:$LISTEN_PORT/healthz")
assert_match "healthz returns 200" "^200$" "$HEALTH"

# --- Test 2: Initial metrics (no flap yet) ---
echo "=== Test: Initial metrics ==="
METRICS=$(limactl shell "$VM_NAME" curl -s "http://localhost:$LISTEN_PORT/metrics")
...
limactl shell "$VM_NAME" sudo ip neigh replace 10.200.0.2 lladdr aa:bb:cc:dd:ee:01 dev veth-host nud reachable
sleep 1
METRICS=$(limactl shell "$VM_NAME" curl -s "http://localhost:$LISTEN_PORT/metrics")
...
limactl shell "$VM_NAME" sudo ip neigh replace 10.200.0.2 lladdr aa:bb:cc:dd:ee:02 dev veth-host nud reachable
sleep 1
METRICS=$(limactl shell "$VM_NAME" curl -s "http://localhost:$LISTEN_PORT/metrics")
...
limactl shell "$VM_NAME" sudo ip neigh replace 10.200.0.2 lladdr aa:bb:cc:dd:ee:03 dev veth-host nud reachable
sleep 1
METRICS=$(limactl shell "$VM_NAME" curl -s "http://localhost:$LISTEN_PORT/metrics")
...
limactl shell "$VM_NAME" sudo ip neigh replace 10.200.0.2 lladdr aa:bb:cc:dd:ee:03 dev veth-host nud stale
sleep 1
METRICS=$(limactl shell "$VM_NAME" curl -s "http://localhost:$LISTEN_PORT/metrics")
...
limactl shell "$VM_NAME" sudo ip neigh del 10.200.0.2 dev veth-host
sleep 1
limactl shell "$VM_NAME" sudo ip neigh replace 10.200.0.2 lladdr aa:bb:cc:dd:ee:03 dev veth-host nud reachable
sleep 1
METRICS=$(limactl shell "$VM_NAME" curl -s "http://localhost:$LISTEN_PORT/metrics")
...
limactl shell "$VM_NAME" sudo ip neigh del 10.200.0.2 dev veth-host
sleep 1
limactl shell "$VM_NAME" sudo ip neigh replace 10.200.0.2 lladdr aa:bb:cc:dd:ee:04 dev veth-host nud reachable
sleep 1
METRICS=$(limactl shell "$VM_NAME" curl -s "http://localhost:$LISTEN_PORT/metrics")
...
limactl shell "$VM_NAME" sudo ip neigh replace 10.200.0.3 lladdr bb:cc:dd:ee:ff:01 dev veth-host nud reachable
sleep 1
METRICS=$(limactl shell "$VM_NAME" curl -s "http://localhost:$LISTEN_PORT/metrics")
...
READY=$(limactl shell "$VM_NAME" curl -s -o /dev/null -w '%{http_code}' "http://localhost:$LISTEN_PORT/readyz")
